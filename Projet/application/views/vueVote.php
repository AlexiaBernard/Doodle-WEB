<?=form_open('Utilisateur/vote',array())?>
	<div class="info2">
		<?php
		...
		<fieldset>
			...
			if( ($heureDebutDiv[0]===$heureFinDiv[0]) && ($heureDebutDiv[1]===$heureFinDiv[1]) ){
					...
					for($a=0;$a<=$nbrejour;$a++){
						echo "<td><input type='checkbox' id='code' name='code[]' value='$heureFinDiv[0]$separateur$heureFinDiv[1]$separateur$a'></td>";
					}
			}else{
				$heureDebutDiv[1]+=1-1;
				$heureFinDiv[1]+=1-1;
				...
				while($i<=$heureFinDiv[0]){
					...
					for($a=0;$a<=$nbrejour;$a++){
						echo "<td><input type='checkbox' id='code' name='code[]' value='$i$separateur$k$separateur$a'></td>";
					}
					echo "</tr>";
					$k+=30;
